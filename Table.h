...
class Deck;
class Card;
template <typename C> class TradeArea;
template <typename C> class Table {
  public:
    ...
    Game game;
    Table(Player &ply1, Player &ply2, Player &ply, Deck &deck,
          DiscardPile<C> &trashT, TradeArea<C> &tradeT)
        : player1(ply1), player2(ply2), player(ply), deckOfC(deck),
          trash(trashT), trade(tradeT), nameFile("game.txt") {
        ...
        if (file.is_open()) { // si le fichier existe
            ...
            if (choice == 1) { // on veut continue le jeu
                ...
                deckOfC = deck;
                trashT = table.trash;
                trash = trashT, tradeT = table.trade;
                trade = tradeT;
    ...
    bool win(string &winnername) {
        if (deckOfC.size() == 0) { // Si le Deck est vide
            if (player1.getNumCoins() > player2.getNumCoins())
                return winnername == player1.getName(); // Le joueur 1 à gagner
            else if (player2.getNumCoins() > player1.getNumCoins())
                return winnername == player2.getName(); // Le joueur 2 à gagner
            else
    ...
    void updateTable(Player &playe, Player &playe1, Player &playe2,
                     DiscardPile<Card *> &dr, TradeArea<Card *> &tr,
                     Deck &decOfC) {
        player = playe;
        player1 = playe1;
        player2 = playe2;
    ...
    friend ostream &operator<<(ostream &os, const Table &table) {
        ...
        os << "Trade Area: " << endl
           << table.trade << endl; // Affiche la zone commerciale*/
        os << "------------------END-TABLE--------------------" << endl;
        return os;
    }
    // affiche les joueurs
    ...
    void startNewGame(Player &ply, Player &play1, Player &play2, Deck &deck) {
        ...
        for (int index = 0; index < 5; index++)
            play1.addCardInHand(deck.draw());
        for (int index = 0; index < 5; index++)
            play2.addCardInHand(deck.draw());
        player1 = play1;
        player2 = play2;
        deckOfC = deck;
        ply = play1;
    ...
    void deleteGame() { filesystem::remove(nameFile); }
    ...
    void addCardsTurn() {
        for (int index = 0; index < 2; index++)
            player.addCardInHand(deckOfC.draw());
    }
};
